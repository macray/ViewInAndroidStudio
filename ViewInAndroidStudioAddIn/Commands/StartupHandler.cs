	...
	public class StartupHandler : CommandHandler
	{
		...
		private void CheckProjectFileName(ProjectFile file)
		{
			...
			if (file.IsResource())
			{
				...
				{
					TaskService.Errors.Add(new TaskListEntry(file.FilePath, "Layout file has axml extension", 0, 0, TaskSeverity.Warning, TaskPriority.Normal, null, this));
				}
				if (file.FilePath.FileName.Any(Char.IsUpper))
				{
					TaskService.Errors.Add(new TaskListEntry(file.FilePath, "Resource file has captial letter in filename", 0, 0, TaskSeverity.Warning, TaskPriority.Normal, null, this));
				}
			}
		}
		...
		private void CheckDirectoryName(FilePath fp)
		{
			...
			{
				TaskService.Errors.Add(new TaskListEntry(fp, "Resource folder has capital letter in filename", 0, 0, TaskSeverity.Warning, TaskPriority.Normal, null, this));
			}
		}
		...
		private void CheckProjectNamingConventions()
		{
			...
			projects = projects.Where(Util.ProjectExtensions.IsAndroidProject);
